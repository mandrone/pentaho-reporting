...
public class FileDataSourceMgmtWriter implements DataSourceMgmtWriter
{
  ...
  private void write(final IDatabaseConnection databaseConnection, final XmlWriter writer) throws IOException
  {
    ...
    {
      rootAttrs.setAttribute(ConnectionModule.NAMESPACE, PROP_TYPE, databaseConnection.getDatabaseType().getShortName());
    }
    final String port = StringUtils.isEmpty(databaseConnection.getDatabasePort()) ? "0" : databaseConnection.getDatabasePort();
    ...
    rootAttrs.setAttribute(ConnectionModule.NAMESPACE, PROP_CONTYPE, setNull(databaseConnection.getAccessType().getName()));
    rootAttrs.setAttribute(ConnectionModule.NAMESPACE, PROP_HOST_NAME, setNull(databaseConnection.getHostname()));
    rootAttrs.setAttribute(ConnectionModule.NAMESPACE, PROP_DATABASE_NAME, setNull(databaseConnection.getDatabaseName()));
    rootAttrs.setAttribute(ConnectionModule.NAMESPACE, PROP_PORT, String.valueOf(new Long(port))); // implicit validate, as the interface allows text here.
    ...
    rootAttrs.setAttribute(ConnectionModule.NAMESPACE, PROP_SERVERNAME, setNull(databaseConnection.getInformixServername()));
    rootAttrs.setAttribute(ConnectionModule.NAMESPACE, PROP_DATA_TBS, setNull(databaseConnection.getDataTablespace()));
    rootAttrs.setAttribute(ConnectionModule.NAMESPACE, PROP_INDEX_TBS, setNull(databaseConnection.getIndexTablespace()));
